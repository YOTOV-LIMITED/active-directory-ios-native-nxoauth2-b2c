...
NSString *const NXOAuth2ConnectionDidStartNotification =
    @"NXOAuth2ConnectionDidStartNotification";
NSString *const NXOAuth2ConnectionDidEndNotification =
    @"NXOAuth2ConnectionDidEndNotification";
...
- (void)applyParameters:(NSDictionary *)parameters
              onRequest:(NSMutableURLRequest *)request;
...
- (BOOL)isServerCertificateForAuthenticationChallenge:
            (NSURLAuthenticationChallenge *)challenge
                                          andHostname:(NSString *)hostname
                                  matchingCertificate:(NSData *)derCertData;
@property(nonatomic, unsafe_unretained, readonly) id<NXOAuth2TrustDelegate>
    trustDelegate;
...
- (id)initWithRequest:(NSMutableURLRequest *)aRequest
    ...
    sendingProgressHandler:
        (NXOAuth2ConnectionSendingProgressHandler)aSendingProgressHandler
           responseHandler:(NXOAuth2ConnectionResponseHandler)aResponseHandler;
{
  self = [self initWithRequest:aRequest
             requestParameters:someRequestParameters
                   oauthClient:aClient
                      delegate:nil];
  if (self) {
...
- (void)dealloc;
{
  if (sendConnectionDidEndNotification)
    [[NSNotificationCenter defaultCenter]
        postNotificationName:NXOAuth2ConnectionDidEndNotification
                      object:self];
  sendConnectionDidEndNotification = NO;
...
- (id<NXOAuth2TrustDelegate>)trustDelegate;
{
  ...
  // The second choice
  if (client &&
      [client.delegate conformsToProtocol:@protocol(NXOAuth2TrustDelegate)]) {
...
- (NSURLConnection *)createConnection;
{
  // if the request is a token refresh request don't sign it and don't check for
  // the expiration of the token (we know that already)
  NSString *oauthAuthorizationHeader = nil;
  if (client.accessToken &&
      ![[requestParameters objectForKey:@"grant_type"]
          isEqualToString:@"refresh_token"]) {
    ...
    oauthAuthorizationHeader = [NSString
        stringWithFormat:@"%@ %@", tokenType, client.accessToken.accessToken];
  }
  ...
  if (oauthAuthorizationHeader) {
    [startRequest setValue:oauthAuthorizationHeader
        forHTTPHeaderField:@"Authorization"];
  }

  if (client.userAgent &&
      ![startRequest valueForHTTPHeaderField:@"User-Agent"]) {
  ...
  NSURLConnection *aConnection =
      [[NSURLConnection alloc] initWithRequest:startRequest
                                      delegate:self
                              startImmediately:NO]; // don't start yet
  [aConnection
      scheduleInRunLoop:[NSRunLoop currentRunLoop]
                forMode:
                    NSDefaultRunLoopMode]; // let's first schedule it in the
                                           // current runloop. (see
                                           // http://github.com/soundcloud/cocoa-api-wrapper/issues#issue/2
                                           // )
  [aConnection start]; // now start

  if (!sendConnectionDidEndNotification)
    [[NSNotificationCenter defaultCenter]
        postNotificationName:NXOAuth2ConnectionDidStartNotification
                      object:self];
...
- (NSString *)descriptionForRequest:(NSURLRequest *)aRequest;
{
  ...
  return [NSString
      stringWithFormat:@"%@ [%@]", aRequest.URL.absoluteString, range];
}
  ...
  if ([httpMethod caseInsensitiveCompare:@"POST"] != NSOrderedSame &&
    ...
    if ([contentType isEqualToString:@"multipart/form-data"]) {
      ...
      [aRequest setHTTPBodyStream:postBodyStream];
               ...
               [contentType
                   isEqualToString:@"application/x-www-form-urlencoded"]) {

      // sends the POST/PUT request as application/x-www-form-urlencoded
      contentType =
          [NSString stringWithFormat:@"application/x-www-form-urlencoded"];
      NSString *query =
          [[aRequest.URL nxoauth2_URLByAddingParameters:parameters] query];
                        ...
                        withTrustMode:(NXOAuth2TrustMode)trustMode;
{
  ...
  if (trustMode & NXOAuth2TrustModeSystem) {
    ...
    OSStatus ossTrust = SecTrustEvaluate(challenge.protectionSpace.serverTrust,
                                         &trustEvalResult);
  ...
  if (trustMode & NXOAuth2TrustModeSpecificCertificate) {
    NSAssert([self.trustDelegate
                 respondsToSelector:@selector(connection:
                                        trustedCertificatesForHostname:)],
             @"For NXOAuth2TrustModeSpecificCertificate the delegate needs to "
             @"implement "
             @"oauthConnection:trustedCertificatesDERDataForHostname:");
    NSArray *trustedCerts = [self.trustDelegate connection:self
                            trustedCertificatesForHostname:hostname];
                                  ...
                                  matchingCertificate:(NSData *)derCertData;
{
  ...
  SecCertificateRef anchorCert =
      SecCertificateCreateWithData(NULL, (__bridge CFDataRef)derCertData);

  if (anchorCert == nil) {
    return NO;
  }

  CFArrayRef allTrustedCert =
      (__bridge CFArrayRef)[NSArray arrayWithObject:(__bridge id)anchorCert];
  ...
  if (checkResult == kSecTrustResultProceed ||
    ...
    for (CFIndex index = 0; index < SecTrustGetCertificateCount(serverTrust);
         index++) {
    ...
    errGetTrustResult = SecTrustGetResult(serverTrust, &checkResult,
                                          &cfCertificates, &statusChain);
    ...
    if (errGetTrustResult == noErr) {
      ...
      for (id certificate in certificates) {
        CFDataRef certData =
            SecCertificateCopyData((__bridge SecCertificateRef)certificate);

        NSString *certificateChecksum =
            [(__bridge_transfer NSData *)certData nx_SHA1Hexdigest];
...
{
  if ([delegate respondsToSelector:@selector(oauthConnection:
                                                didSendBytes:
                                                     ofTotal:)]) {
...
didReceiveResponse:(NSURLResponse *)theResponse;
{
#if (NXOAuth2ConnectionDebug)
  NSLog(@"%.0fms (RESP) - %@", -[startDate timeIntervalSinceNow] * 1000.0,
        [self descriptionForRequest:request]);
#endif
  ...
  if (/*self.statusCode == 401 // TODO: check for status code once the bug returning 500 is fixed
      ...
      authenticateHeader &&
      [authenticateHeader rangeOfString:@"expired_token"].location !=
          NSNotFound) {
    [self cancel];
    [client refreshAccessTokenAndRetryConnection:self];
  } else {
    if ([delegate
            respondsToSelector:@selector(oauthConnection:didReceiveData:)]) {
      [delegate oauthConnection:self
                 didReceiveData:data]; // inform the delegate that we start with
                                       // empty data
    }
    if ([delegate respondsToSelector:@selector(oauthConnection:
                                            didReceiveResponse:)]) {
...
{
#if (NXOAuth2ConnectionDebug)
  NSLog(@"%.0fms (SUCC) - %@", -[startDate timeIntervalSinceNow] * 1000.0,
        [self descriptionForRequest:request]);
#endif
  ...
  if (self.statusCode < 400) {
    if ([delegate
            respondsToSelector:@selector(oauthConnection:didFinishWithData:)]) {
    ...
    if (self.statusCode == 401) {
      ...
      if ([response isKindOfClass:[NSHTTPURLResponse class]]) {
        NSDictionary *headerFields =
            [(NSHTTPURLResponse *)response allHeaderFields];
        for (NSString *headerKey in headerFields.allKeys) {
          if ([[headerKey lowercaseString]
                  isEqualToString:@"www-authenticate"]) {
            authenticateHeader = [headerFields objectForKey:headerKey];
            break;
          }
        }
      ...
      if (authenticateHeader &&
          [authenticateHeader rangeOfString:@"invalid_token"].location !=
              NSNotFound) {
    ...
    NSString *localizedError =
        [NSString stringWithFormat:NSLocalizedString(
                                       @"HTTP Error: %d",
                                       @"NXOAuth2HTTPErrorDomain description"),
                                   self.statusCode];
    NSDictionary *errorUserInfo =
        [NSDictionary dictionaryWithObject:localizedError
                                    forKey:NSLocalizedDescriptionKey];
    NSError *error = [NSError errorWithDomain:NXOAuth2HTTPErrorDomain
                                         code:self.statusCode
                                     userInfo:errorUserInfo];
    if ([delegate
            respondsToSelector:@selector(oauthConnection:didFailWithError:)]) {
...
#if (NXOAuth2ConnectionDebug)
  NSLog(@"%.0fms (FAIL) - %@ (%@ %i)",
        -[startDate timeIntervalSinceNow] * 1000.0,
        [self descriptionForRequest:request], [error domain], [error code]);
#endif
            ...
            redirectResponse:(NSURLResponse *)aRedirectResponse;
{
...
#if (NXOAuth2ConnectionDebug)
    NSLog(@"%.0fms (WILL) - %@", -[startDate timeIntervalSinceNow] * 1000.0,
          [self descriptionForRequest:aRequest]);
#endif
    return aRequest; // if not redirecting do nothing
  }

  if ([delegate respondsToSelector:@selector(oauthConnection:
                                       didReceiveRedirectToURL:)]) {
    [delegate oauthConnection:self didReceiveRedirectToURL:aRequest.URL];
  }

#if (NXOAuth2ConnectionDebug)
  NSLog(@"%.0fms (REDI) - %@ > %@", -[startDate timeIntervalSinceNow] * 1000.0,
        aRedirectResponse.URL.absoluteString,
        [self descriptionForRequest:aRequest]);
#endif
  BOOL hostChanged =
      [aRequest.URL.host caseInsensitiveCompare:aRedirectResponse.URL.host] !=
      NSOrderedSame;

  BOOL schemeChanged =
      [aRequest.URL.scheme
          caseInsensitiveCompare:aRedirectResponse.URL.scheme] != NSOrderedSame;
  BOOL schemeChangedToHTTPS =
      schemeChanged &&
      ([aRequest.URL.scheme caseInsensitiveCompare:@"https"] == NSOrderedSame);
  ...
  if (hostChanged || (schemeChanged && !schemeChangedToHTTPS)) {
    [mutableRequest
                  setValue:nil
        forHTTPHeaderField:@"Authorization"]; // strip Authorization information
    ...
    totalBytesExpectedToWrite:(NSInteger)totalBytesExpectedToWrite;
{
  if ([delegate respondsToSelector:@selector(oauthConnection:
                                                didSendBytes:
                                                     ofTotal:)]) {
    [delegate oauthConnection:self
                 didSendBytes:totalBytesWritten
                      ofTotal:totalBytesExpectedToWrite];
  }

  if (sendingProgressHandler)
    sendingProgressHandler(totalBytesWritten, totalBytesExpectedToWrite);
}

- (NSInputStream *)connection:(NSURLConnection *)connection
            needNewBodyStream:(NSURLRequest *)aRequest;
{
    ...
    didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge;
{
  if ([challenge.protectionSpace.authenticationMethod
    ...
    if ([self.trustDelegate
            respondsToSelector:@selector(connection:trustModeForHostname:)]) {
      effectiveTrustMode =
          [self.trustDelegate connection:self trustModeForHostname:hostname];
    }
    BOOL shouldTrustCerificate =
        [self trustsAuthenticationChallenge:challenge
